...
dag = DAG(
    'fb_ads_extended_pipe',
    default_args=default_args,
    description='A dag to dump and mark our database with ads that have any activity in last N days',
...
#     dt5=(date.today()-timedelta(5))
    ...
    days_back =  datetime.timedelta(589)
    date_since = datetime.date.today() - days_back
    
    days_till = datetime.timedelta(409)
    date_upto = datetime.date.today() - days_till
    ## -----------------------------
    ...
    facebook_params = {
        ...
        "ad_delivery_date_max": str(date_upto),
...
#     valid_candidates = [candidate for candidate in mongo_database["cand_info"].find({"drop_date": float("NaN")})] 
    valid_candidates = [candidate for candidate in mongo_database["cand_info"].find({"drop_date": float("NaN")}) if candidate["candidate_name"] == "Donald J. Trump"]
    ## -----------------------------
...
#                     try:
#                         try:
#                             if datetime.strptime(resp_json["data"][-1]["ad_delivery_stop_time"][0:10], '%Y-%m-%d').date()>dt5 or datetime.strptime(resp_json["data"][-1]["ad_delivery_stop_time"][0:10], '%Y-%m-%d').date()==None or datetime.strptime(resp_json["data"][-1]["ad_delivery_stop_time"][0:10], '%Y-%m-%d').date()=='NaN':    
#                                 break
#                         except KeyError:
...
def push_data(ds, **kwargs):
    print("Pushing")
    ct=0
    for doc in db['fb_ads_extended_dump'].find({'$or':[{'marked':1}, {'marked':0}, {'marked_topic':0}, {'marked_topic':1}]},no_cursor_timeout=True):
        ct+=1
        if ct%1000==0:
            print(ct)
