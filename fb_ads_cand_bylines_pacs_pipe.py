...
dag = DAG(
    ...
    description='A dag to update our database with fb ads for candidates running ads on other pages bylines',
...
col=db["fb_ads_cand_bylines_pacs_dump"]
data=[]
...
#     dt5=(date.today()-timedelta(5))
    ...
    ## -----------------------------
    valid_candidates = [candidate for candidate in mongo_database["cand_info_extended"].find({"drop_date": float("NaN")})]
    ## -----------------------------
...
def push_data(ds, **kwargs):
    print("Pushing")
    ct=0
    for doc in db['fb_ads_cand_bylines_pacs_dump'].find({'$and':[{'$or':[{'marked':1}, {'marked':0}, {'marked_topic':0}, {'marked_topic':1}]}, {'ad_delivery_start_time':{'$gte':'2020-06-01'}}, {'$or':[{'funding_entity':'TRUMP MAKE AMERICA GREAT AGAIN COMMITTEE'},
