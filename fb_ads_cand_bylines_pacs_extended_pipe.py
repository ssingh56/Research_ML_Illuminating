...
default_args = {
    ...
    'start_date': datetime(2020,7,31,23,0,0),
...
dag = DAG(
    ...
    description='A dag to update our database with fb ads for candidates running ads that have any activity in last N days on other pages bylines',
...
col=db["fb_ads_cand_bylines_pacs_extended_dump"]
data=[]
...
#     dt5=(date.today()-timedelta(5))
    ...
    days_back =  datetime.timedelta(40)
    ...
    ## -----------------------------
    valid_candidates = [candidate for candidate in mongo_database["cand_info_extended"].find({"drop_date": float("NaN")}) if candidate["page_name"] == "PopSugar" or candidate["page_name"] == "IGN"]
    ## -----------------------------

    for candidate in valid_candidates:
        ad_buys_processed = 0
        print(candidate["page_name"])
...
#     print("hellooo")
#     t=time.time()
#     for page in db['cand_info_extended'].find(no_cursor_timeout=True):
...
# page_ids=['6726182861','1771156219840594',
#           '103597604713894','1745691098822972',
#           '607626319739286','612701059241880',
#           '147542771956309','706716899745696',
#           '100560711450721','105370081104608',
#           '12301006942', '529594453913030',
#           '24413227922', '105235674308900',
#           '105993567885941', '438007310446785',
#           '437400020507592', '104940277994339',
#           '105059594648138', '103315431491657',
#           '103169218173211', '101654774973359',
#           '114904113668632', '102940748211235',
#           '100556838454730', '100543878456307',
#           '109131637582529', '109944567501859',
#           '105479921284817', '112479700577922',
#           '105404247960680', '114003843758623',
#           '104354594734685', '107931061038492'
#           ]
...
#     mx_dt='2020-01-01'
#     col.remove()
#     dt_stop=(date.today()-timedelta(10))
#     dt_start=(date.today()-timedelta(10))
...
#                 try:
#                     try:
#                         if datetime.strptime(resp_json["data"][-1]["ad_delivery_stop_time"][0:10], '%Y-%m-%d').date()<dt_stop:    
#                             break
#                     except KeyError:
#                         if datetime.strptime(resp_json["data"][-1]["ad_delivery_start_time"][0:10], '%Y-%m-%d').date()<dt_start:
#                             break
#                 except IndexError:
#                     try:
#                         x["data"][-1]["ad_delivery_stop_time"][0:10]
...
col1=db["fb_ads_cand_bylines_pacs"]
def push_data(ds, **kwargs):
    print("Pushing")
    ct=0
    for doc in db['fb_ads_cand_bylines_pacs_extended_dump'].find({'$and':[{'$or':[{'marked':1}, {'marked':0}, {'marked_topic':0},{'marked_topic':1}]}, {'ad_delivery_start_time':{'$gte':'2020-06-01'}}, {'$or':[{'funding_entity':'TRUMP MAKE AMERICA GREAT AGAIN COMMITTEE'},
        {'funding_entity':'the Trump Make America Great Again Committee'}, 
        {'funding_entity':'DONALD J. TRUMP FOR PRESIDENT, INC.'},
        {'funding_entity':'Donald J. Trump for President, Inc.'},
        {'funding_entity':'BIDEN VICTORY FUND'},
        {'funding_entity':'BIDEN FOR PRESIDENT'},
        {'funding_entity':'Trump Make America Great Again Committee'},
        {'funding_entity':'Biden for President'}]}]},no_cursor_timeout=True):
        ct+=1
        if ct%1000==0:
            print(ct)
