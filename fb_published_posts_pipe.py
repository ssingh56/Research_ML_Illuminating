...
col=db["fb_published_posts_dump"]
data=[]
...
#         ann_dt=datetime.strptime(ann_dt, '%m/%d/%Y').date()
        if y==1:
            params = (('fields', 'id,message,created_time,updated_time,shares,likes.limit(1).summary(true),comments.limit(1).summary(true)'),('since',enddt.__str__()),('until',startdt.__str__()),('include_hidden','true'),('limit',5),('access_token', token))
            resp = requests.get('https://graph.facebook.com/v3.1/' + page['page_id'] + '/published_posts', params=params)
...
def push_data(ds, **kwargs):
    ...
    ct=0
    #{'marked':1},
    for doc in db['fb_published_posts_dump'].find(no_cursor_timeout=True):
        ct+=1
        if ct%100==0:
            print(ct)
