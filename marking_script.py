...
database = mongoClient[dbase]
db=database['fb_ads_backfill']

default_args = {
    'owner': 'airflow',
    'depends_on_past': False,
    'start_date': datetime(2020,3,12,0,0,0),
    ...
    'execution_timeout': timedelta(minutes=420),
...
def predict(ds, **kwargs):
    ...
    data=[]
    ls=[]
    for doc in db.distinct('ad_creative_body',{'marked':0}):
        if len(data)<5000:
            data.append({'ad_creative_body':doc})
        else:
            break           
#     for doc in db.find({'marked':0}):
#         if len(data)<2000:
#             if doc['ad_creative_body'] not in ls:
#                 doc['marked']=1
#                 ls.append(doc['ad_creative_body'])
#                 if db.find_one({'ad_creative_body':doc['ad_creative_body'],'marked':1}):
#                     x=db.find_one({'ad_creative_body':doc['ad_creative_body'],'marked':1})
#                     doc['class']=x['class']
#                     db.update_one({'_id': doc['_id']},{"$set":doc},True)
#                 else:
#                     data.append(doc)
#                     if len(data)%500==0:
#                         print("500")
...
#             if __name__ ==  '__main__':
        ...
        ct=0
        for doc in dc:
            for x in db.find({"ad_creative_body":doc['ad_creative_body'],'marked':0}):
                ct+=1
                if ct%1000==0:
                    print(ct)
                x['marked']=1
                x['class']=doc['class']
                db.update_one({'_id': x['_id']},{"$set":x},True)    
        return "Done"
    
dag = DAG(
    'fb_ads_marking',
    default_args=default_args,
    description='A dag to mark our database with new ads using BERT',
