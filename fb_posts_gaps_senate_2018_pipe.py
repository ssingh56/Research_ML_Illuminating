...
default_args = {
    ...
    'start_date': datetime(2018,9,10,0,0,0),
    ...
    # 'pool': 'backfill',
    # 'priority_weight': 10,
#     'end_date': datetime(2018,11,14,0,0,0),
...
dag = DAG(
    ...
    description='A dag to dump and mark our database with missing senate 2018 facebook posts',
...
col=db["fb_posts_gaps_2018_dump"]
data=[]
...
def get_data(ds, **kwargs):
    ...
    col.remove()
    fail=[]
    dt=(date.today()-timedelta(726))
    dt5=(date.today()-timedelta(791))
    resp={}
    print("hellooo")
    t=time.time()
    for page in db['cand_info_gaps_2018'].find(no_cursor_timeout=True):
...
#         ann_dt=datetime.strptime(ann_dt, '%m/%d/%Y').date()
        if y==1:
            ...
            resp = requests.get('https://graph.facebook.com/v3.1/' + page['fb_account_handle'] + '/posts', params=params)
...
col1=db["fb_posts_gaps_2018_dev"]
def push_data(ds, **kwargs):
    print("Pushing")
    ct=0
    #{'marked':1},
    for doc in db['fb_posts_gaps_2018_dump'].find(no_cursor_timeout=True):
        ct+=1
        if ct%100==0:
            print(ct)
...
# get_data_step >> marking_step >> push_data_step >> stop_op
